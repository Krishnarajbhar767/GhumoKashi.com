    ...
    <div className="w-[80%] py-5 flex flex-col items-center gap-5 ">
      ...
      <div className="w-full  h-fit flex flex-col gap-5">
        <div className="firstRow w-full grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1  gap-5 ">
          {
            row1?.map((item, i) => (
              <LazyLoad>
                <img
                  className="hover:opacity-55 rounded-lg cursor-pointer"
                  src={item.url}
                  alt="galleryImg"
                />
              </LazyLoad>
        ...
        <div className="firstRow w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4  gap-5 ">
          {
            row3?.map((item, i) => (
              <div className=" bg-center bg-cover h-[45vh] w-full hover:cursor-pointer flex items-center rounded-lg hover:opacity-75 justify-center " style={{ backgroundImage: `url(${item.url})` }}>
