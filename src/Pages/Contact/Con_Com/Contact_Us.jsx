...
const Contact_Us = () => {
    ...
    const onSubmit = (data) => {
        emailjs
            .send("service_hw8adtt", "template_58fv2xh", data, "ORPdMtnsw34TNtIO4")
            .then(
                () => {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="max-w-lg mx-auto p-6 rounded-2xl space-y-4">
            <div>
            ...
            <div>
                ...
                <input
                    type="number"
                    max="10"
                    {...register("from_phone", { required: "Phone number is required" })}
                    className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500"
                />
                {errors.from_phone && <p className="text-red-500 text-sm">{errors.from_phone.message}</p>}
            </div>
